...
!****************************************************************************
      ...
      MODULE GLOBAL_VAR
        REAL*8:: g,at0,at1,at2,at3,b,r0,v0,uL,uT
    ...
    OPEN(UNIT=300,FILE='t300.dat')


    uL=6978136.d0
    uT=sqrt(6978136.d0**3/3.986004415d14)

 
    g=1.d0
    b=1.53055116d-14*(uL)*1.d3!é igual à 1.06804d-4
    at0=1.d0
    at1=2.d-4
    at2=0.d-4
    at3=0.d-4
...
x=0.d0    
x0=0.999999999999463d0-(5.d3)/uL
yu0=6.718379990556269d-013
p0=-7.042331046609752d-013
q0=1.00000000000063d0
...
80      continue
        ...
        do 90 k=1,n
            cum(k)=cum(k)+dlog(znorm(k))/dlog(2.d0)
90      continue
        ...
        if (mod(i,i0).eq.0) then
            ...
            !$omp critical
            type*,"atual=",i/i0,"/",nmax/i0
            !write(*,*)x,(cum(k)/x,k=1,N)
            explyap=max(cum(1),cum(2),cum(3),cum(4))/x
            
            if (i/i0.gt.1) then
                cm=explyap-explyap0
                iprevi=int(i/i0-int(explyap0/cm,8))
                type*,"previsao linear=",iprevi
            end if
            write(*,*)"r/r0=",dsqrt(y(1)**2+y(3)**2)/dsqrt(x0**2+yu0**2)
            
            explyap0=explyap
            
            type*,"explyap=",explyap
            write(103,*),explyap
            
            
            
            
            raio=dsqrt(y(1)**2+y(3)**2)
            write(300,*)x*uT/(3600.d0*24.d0*365.d0),(raio-r0)*uL/1000.d0
        
            
            !pause
            !    write(*,*)"raio",sqrt(y(1)**2+y(3)**2)
            !    write(*,*)"raio0",sqrt(x0**2+yu0**2)
            !    write(*,*)"v",sqrt(y(2)**2+y(4)**2)
            !    write(*,*)"v0",sqrt(p0**2+q0**2)
            !pause
            !$omp end critical
            if (i.eq.nmax) then
                !$omp critical
                ...
                type*,"maxlyap=",explyap
                write(*,*)"raio",dsqrt(y(1)**2+y(3)**2)
                write(*,*)"raio0",dsqrt(x0**2+yu0**2)
                write(*,*)"v",dsqrt(y(2)**2+y(4)**2)
                write(*,*)"v0",dsqrt(p0**2+q0**2)
                pause
                !$omp end critical
            end if
        end if
...
100 continue 
    ...
    USE GLOBAL_VAR
    IMPLICIT real*8(A-H,P-Z)
    dimension y(20),yprime(20)
    !equações de movimento:

yprime(1)=y(2)
yprime(2)=-((g*y(1))/(y(1)**2 + y(3)**2)**1.5) - b*y(2)*Sqrt(y(2)**2 + y(4)**2) - (y(3)*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/Sqrt(y(1)**2 + y(3)**2)
yprime(3)=y(4)
yprime(4)=-((g*y(3))/(y(1)**2 + y(3)**2)**1.5) - b*y(4)*Sqrt(y(2)**2 + y(4)**2) + (y(1)*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/Sqrt(y(1)**2 + y(3)**2)
...
yprime(9+i)=        ((3*g*y(1)**2)/(y(1)**2 + y(3)**2)**2.5 - g/(y(1)**2 + y(3)**2)**1.5 - (y(3)*(-((at1*y(1))/Sqrt(y(1)**2 + y(3)**2)) - (3*at2*y(1)*(r0 - Sqrt(y(1)**2 + y(3)**2))**2)/Sqrt(y(1)**2 + y(3)**2) - (at3*y(1)*Sqrt(y(2)**2 + y(4)**2))/(y(1)**2 + y(3)**2)**1.5))/Sqrt(y(1)**2 + y(3)**2) + &
          (y(1)*y(3)*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/(y(1)**2 + y(3)**2)**1.5)*y(5 + i) + &
       (-((b*y(2)**2)/Sqrt(y(2)**2 + y(4)**2)) - (at3*y(2)*y(3))/((y(1)**2 + y(3)**2)*Sqrt(y(2)**2 + y(4)**2)) - b*Sqrt(y(2)**2 + y(4)**2))*y(9 + i) + &
       ((3*g*y(1)*y(3))/(y(1)**2 + y(3)**2)**2.5 - (y(3)*(-((at1*y(3))/Sqrt(y(1)**2 + y(3)**2)) - (3*at2*y(3)*(r0 - Sqrt(y(1)**2 + y(3)**2))**2)/Sqrt(y(1)**2 + y(3)**2) - (at3*y(3)*Sqrt(y(2)**2 + y(4)**2))/(y(1)**2 + y(3)**2)**1.5))/Sqrt(y(1)**2 + y(3)**2) + &
          (y(3)**2*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/(y(1)**2 + y(3)**2)**1.5 - &
          (at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2))/Sqrt(y(1)**2 + y(3)**2))*y(13 + i) + &
       (-((b*y(2)*y(4))/Sqrt(y(2)**2 + y(4)**2)) - (at3*y(3)*y(4))/((y(1)**2 + y(3)**2)*Sqrt(y(2)**2 + y(4)**2)))*y(17 + i)
     
yprime(13+i)=y(17 + i)

yprime(17+i)=        ((3*g*y(1)*y(3))/(y(1)**2 + y(3)**2)**2.5 + (y(1)*(-((at1*y(1))/Sqrt(y(1)**2 + y(3)**2)) - (3*at2*y(1)*(r0 - Sqrt(y(1)**2 + y(3)**2))**2)/Sqrt(y(1)**2 + y(3)**2) - (at3*y(1)*Sqrt(y(2)**2 + y(4)**2))/(y(1)**2 + y(3)**2)**1.5))/Sqrt(y(1)**2 + y(3)**2) - &
          (y(1)**2*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/(y(1)**2 + y(3)**2)**1.5 + &
          (at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2))/Sqrt(y(1)**2 + y(3)**2))*y(5 + i) + &
       ((at3*y(1)*y(2))/((y(1)**2 + y(3)**2)*Sqrt(y(2)**2 + y(4)**2)) - (b*y(2)*y(4))/Sqrt(y(2)**2 + y(4)**2))*y(9 + i) + &
       ((3*g*y(3)**2)/(y(1)**2 + y(3)**2)**2.5 - g/(y(1)**2 + y(3)**2)**1.5 + (y(1)*(-((at1*y(3))/Sqrt(y(1)**2 + y(3)**2)) - (3*at2*y(3)*(r0 - Sqrt(y(1)**2 + y(3)**2))**2)/Sqrt(y(1)**2 + y(3)**2) - (at3*y(3)*Sqrt(y(2)**2 + y(4)**2))/(y(1)**2 + y(3)**2)**1.5))/Sqrt(y(1)**2 + y(3)**2) - &
          (y(1)*y(3)*(at0*b*v0**2 + at1*(r0 - Sqrt(y(1)**2 + y(3)**2)) + at2*(r0 - Sqrt(y(1)**2 + y(3)**2))**3 + (at3*Sqrt(y(2)**2 + y(4)**2))/Sqrt(y(1)**2 + y(3)**2)))/(y(1)**2 + y(3)**2)**1.5)*y(13 + i) + &
       ((at3*y(1)*y(4))/((y(1)**2 + y(3)**2)*Sqrt(y(2)**2 + y(4)**2)) - (b*y(4)**2)/Sqrt(y(2)**2 + y(4)**2) - b*Sqrt(y(2)**2 + y(4)**2))*y(17 + i)
...
10  continue
    ...
    return
      ...
      integer n, ind, nw, k
      real*8 x, y(n), xend, tol, c(1), w(nw,9), temp
!c
!c***********************************************************************
!c                                                                      *
